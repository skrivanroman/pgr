...
class Ground
{
 public:
  ...
  Ground(float size, uint32_t resolution, float textureTiling, const Location& location,
 ...
 private:
  void generate(float size, uint32_t resolution, float textureTiling)
  {
    ...
    for (uint32_t z = 0; z <= resolution; ++z)
    {
      for (uint32_t x = 0; x <= resolution; ++x)
      {
        ...
        v.position = glm::vec3(-half + xf * size, 0.0f, -half + zf * size);

        v.normal = glm::vec3(0.0f, 1.0f, 0.0f);

        v.texCoords = glm::vec2(xf * textureTiling, zf * textureTiling);
  ...
  void setupMesh(const Location& location)
  {
    ...
    glVertexAttribPointer(location.position, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
